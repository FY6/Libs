...
public class AbImageUtil {
    ...
    public static int getMipIdBy(String name) {
        int id = RxRetrofitApp.getApplication().getResources().getIdentifier(name, "mipmap", RxRetrofitApp.getApplication()
                .getPackageName());
        return id;
    }
     ...
     * @return
     */
    public static Drawable getAstsBy(String name) {
        //测试一:获取asset下图片资源
        try {
            AssetManager assetManager = RxRetrofitApp.getApplication().getAssets();
    ...
    public static Bitmap getMagicDrawingCache(View view, boolean quick_cache) {
        ...
        if (view.getWidth() == 0 && view.getHeight() == 0) {
            view.measure(MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED), MeasureSpec.makeMeasureSpec(0, MeasureSpec
                    .UNSPECIFIED));
            view.layout(0, 0, view.getMeasuredWidth(), view.getMeasuredHeight());
        }
